# Production-like security groups for staging environment
variable "vpc_id" {
...
resource "aws_security_group" "web_tier" {
  name_prefix = "staging-use1-web-sg-"
  ...
  # Allow React development server for testing
  ingress {
    description = "React dev server for staging testing"
    from_port   = 3000
    to_port     = 3000
    protocol    = "tcp"
    cidr_blocks = [var.vpc_cidr]
...
resource "aws_security_group" "app_tier" {
  name_prefix = "staging-use1-app-sg-"
  ...
  # Allow React app from web tier
  ingress {
    description     = "React app from web tier"
    from_port       = 3000
    to_port         = 3000
    protocol        = "tcp"
    security_groups = [aws_security_group.web_tier.id]
...
resource "aws_security_group" "database_tier" {
  name_prefix = "staging-use1-db-sg-"
...
resource "aws_security_group" "eks_cluster" {
  name_prefix = "staging-use1-eks-cluster-sg-"
  ...
  tags = merge(var.tags, {
    Name = "staging-use1-eks-cluster-sg"
    Tier = "kubernetes"
    Component = "cluster"
  })
}
...
resource "aws_security_group" "eks_nodes" {
  name_prefix = "staging-use1-eks-nodes-sg-"
  ...
  tags = merge(var.tags, {
    Name = "staging-use1-eks-nodes-sg"
    Tier = "kubernetes"
    Component = "nodes"
  })
}

# Simple consolidated security group (similar to dev but with production-like rules)
resource "aws_security_group" "staging_basic" {
  name_prefix = "staging-use1-basic-sg-"
  description = "Basic security group for staging environment"
  vpc_id      = var.vpc_id

  # Allow SSH access from VPC only (more restrictive than dev)
  ingress {
    description = "SSH from VPC"
    from_port   = 22
    to_port     = 22
  ...
  # Allow staging HTTP port
  ingress {
    description = "Staging HTTP port"
    from_port   = 8080
    to_port     = 8080
    protocol    = "tcp"
    cidr_blocks = [var.vpc_cidr]
  ...
  # Allow React development server for staging
  ingress {
    description = "React staging server"
    from_port   = 3000
    to_port     = 3000
    protocol    = "tcp"
    cidr_blocks = [var.vpc_cidr]
...
output "basic_security_group_id" {
  description = "ID of the basic security group"
  value       = aws_security_group.staging_basic.id
}

output "security_group_id" {
  description = "ID of the basic security group (for compatibility)"
  value       = aws_security_group.staging_basic.id
}
...
output "security_group_ids" {
  ...
  value = {
    ...
    basic         = aws_security_group.staging_basic.id
  }
}
